...
from ovid_core.authentication import (
    ...
    codex_subscription_provider_definition,
)
...
async def test_codex_provider_definition_derives_flows_and_models(mocker: MockerFixture) -> None:
    store = MemoryCodexTokenStore()
    model_factory = cast(CodexSubscriptionModelFactory, mocker.Mock())
    model_factory.available_options = mocker.AsyncMock(
        return_value=model_selection_options(models=(KnownModel(provider='codex-subscription', model='gpt-codex'),))
    )
    ...
    assert definition.id == 'codex-subscription'
    assert [flow.id for flow in definition.flows] == ['browser', 'device']
    assert (await definition.models()).models[0].value == 'gpt-codex'

    model_factory.available_options.return_value = model_selection_options(
        models=(KnownModel(provider='openai', model='gpt'),)
    )
    with pytest.raises(AuthenticationError, match='catalog is unavailable'):
        await definition.models()
