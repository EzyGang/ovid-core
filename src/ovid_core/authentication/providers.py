...
def codex_subscription_provider_definition(
    *,
    ...
    async def load_models() -> ModelProviderOption:
        options = await model_factory.available_options()
        provider = next((candidate for candidate in options.providers if candidate.value == _CODEX_PROVIDER), None)
        
        if provider is None:
            raise AuthenticationError('Codex subscription model catalog is unavailable')
